r"""
This module implements various VI models for the purpose of SDEs. We assume observations are available at times $t_0, ... t_N$.
However, the user can increase the resolution for which the model can fit the data to increase accuracy, that is, the model will
try to fit the latent at times $t_{0,0}, t_{0,1}, ..., t_{0,M}, t_{1,0}, ..., t_{N-1, M}, t_{N,0}$ for resolution $M$.

To do this at time $t_{n,m}$, we give the model the following arguments (stacked in an array):
  - $y_n$, observation at $t_{n,0}$
  - $y_{n+1}$, observation at $t_{n+1, 0}$.
  - $m$, current time point between observations

Essentially, we give the model the two nearest observations relative to the current time step and the time step. At time $t_{N,0}$, where
there is no $y_{N+1}$, we use $y_N$ twice instead.
...
class ArcherModel:
    ...
    def _par_parse(self, params, y_meas):
        ...
        lower_chol = jax.vmap(lower_model)(jnp.concatenate([y_meas[:-1], y_meas[1:]], axis=1)).reshape((-1, self._n_state, self._n_state)) 
